...
// runtime (cogolf_replay.js, Nim -> emscripten) and the OffscreenCanvas
...
// two cogame-cogolf differences: `init` may carry the replay bytes the
...
function stageNote() {
  ...
  try {
    var length = Module._cogolf_stage_len ? Module._cogolf_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._cogolf_stage_ptr();
...
function runtimeError() {
  var length = Module._cogolf_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._cogolf_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function runtimeProfile() {
  try {
    var length = Module._cogolf_profile_len ? Module._cogolf_profile_len() : 0;
    if (!length) return '';
    var pointer = Module._cogolf_profile_ptr();
...
function ingestPacket() {
  var length = Module._cogolf_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._cogolf_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._cogolf_input(pointer, length);
  });
}
...
async function handOverAtlas() {
  ...
  if (!Module.FS || !Module._cogolf_set_atlas) {
  ...
  var accepted = copyIntoRuntime(image.data, function (pointer) {
    return Module._cogolf_set_atlas(pointer, width, height, 0);
  });
  if (accepted !== 1) {
    bitmap.close();
  ...
  copyIntoRuntime(halfImage.data, function (pointer) {
    return Module._cogolf_set_atlas(pointer, halfW, halfH, 1);
  });
  return true;
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._cogolf_load_replay(pointer, length);
    });
    if (!loaded) throw new Error(runtimeError());
    runtimeLoaded = true;
    perfMark('wasm load_replay done (' + Module._cogolf_packet_len() + ' B first packet)');
...
function advance(frames) {
  ...
  try {
    ...
    for (var i = 0; i < count; i++) {
      if (Module._cogolf_frame() < 0) throw new Error(runtimeError());
      ingestPacket();
    }
    postMessage({
      type: 'advanced',
...
importScripts('./broadcast_core.js', './cogolf_replay.js');
