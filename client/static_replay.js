...
// API the replay page drives (sendCommand, zoomAt, setZoom, panBy, panTo,
// resetView, getTransform, ...). Copied from cogame-factorio
// client/static_replay.js (itself coworld-ctf's) and mechanically renamed:
// FactorioStaticReplay -> CogolfStaticReplay, Module._factorio_* ->
// Module._cogolf_*. The bootstrap is UNTOUCHED — `var Module = {}` +
// `Module.onRuntimeInitialized` + `importScripts` in the Worker, the
// NON-modularized pairing that matches replay-viewer/config.nims (no
// MODULARIZE, no EXPORT_NAME). Mixing the two hangs on "Loading replay…"
// forever (cogame-lantern, 2026-08-23).
//
// Two cogolf changes: `data-replay-loaded="true"` is set on the FIRST DRAWN
// FRAME rather than when the bytes finished parsing (so the attribute means
// "pixels were composited"), and every failure also sets
// `data-replay-error`. tools/ci/viewer_smoke.mjs reads exactly those two
// attributes.
(function () {
  'use strict';
  ...
  function createCore(config) {
    ...
    function onWorkerMessage(event) {
      ...
      try {
        if (message.type === 'text') {
          ...
          // "loaded" means pixels were composited, not bytes parsed.
    ...
    function start(replayBytes) {
      ...
      try {
        worker = new Worker(workerUrl, { name: 'cogolf-static-replay' });
